...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const finishExpedition = () => {
    ...
    updateDisplay();
      displayMessage(`Экспедиция "${gameConfig.EXPEDITION_TYPES[expeditionType]}" завершена! Награда: ${reward} алмазов`, 'green');
  };
  ...
  const buyKey = () => {
    if (gameState.diamonds >= 10) {
      ...
      gameState.keys++;
      updateDisplay();
        displayMessage('Ключ приобретен!');
    } else {
        displayMessage('Недостаточно алмазов для покупки ключа', 'red');
    }
  };

  const buyChest = (chestType) => {
      const cost = chestType === 'common' ? 1 : chestType === 'rare' ? 5 : 10;
    if (gameState.diamonds >= cost) {
      gameState.diamonds -= cost;
      gameState.chests[chestType]++;
        updateDisplay();
      displayMessage(`Сундук ${chestType} приобретен!`);
    } else {
      displayMessage('Недостаточно алмазов для покупки сундука', 'red');
    }
  };
    
    const createItemPopup = (itemType, itemName, name, count, rarity, bonuses = '') => {
    const itemPopup = document.getElementById('item-popup');
    const popupContent = itemPopup.querySelector('.item-popup-content');
        const imagePath = `${itemName}.jpg`;

    popupContent.innerHTML = `
        <h2>${name}</h2>
        <img class="item-image" src="img/${imagePath}" alt="${name}">
        <p>Количество: ${count}</p>
        <p>Редкость: ${rarity}</p>
         ${bonuses ? `<p style="white-space: pre-line;">Бонусы:\n${bonuses}</p>` : ''}
    `;

    itemPopup.style.display = 'block';

    const closePopupBtn = itemPopup.querySelector('.close-popup-button');
